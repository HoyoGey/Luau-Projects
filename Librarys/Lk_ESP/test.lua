...
function ESP:addESP(a,settings)
	...
	function newESP:AddObject(object,title)
        ...
        if object:FindFirstChildOfClass("BillboardGui") and not string.find(object:FindFirstChildOfClass("BillboardGui").Name, title) then
            object:FindFirstChildOfClass("BillboardGui"):Remove()
        end
        for _,v in ipairs(game.CoreGui:GetDescendants()) do
            if v.ClassName == "Highlight" and v.Adornee == object then
                v.Adornee = nil
                v:Remove()
            end
        end
